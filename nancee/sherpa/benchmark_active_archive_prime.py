...
With --prime on, it also sends a silent one-token context-prime request
after each archive cut. The prime cost is recorded separately. The next
real turn shows whether the compact prompt prefix was successfully warmed.
...
def parse_args():
    ...
    parser.add_argument(
        ...
        choices=("on", "off"),
        default="on",
        help=("Silently prime the compact prompt after each archive cut. Default: on"),
    )
...
def run_scenario(name, total_turns, warmup_mode, prime_mode):
    ...
    for turn_number in range(1, total_turns + 1):
        ...
        if name == "archive":
            ...
            if moved:
                ...
                if prime_mode == "on":
                    print(
                        "[BENCHMARK] Priming compact prompt after "
                        f"archiving {len(moved)} turns...",
                        flush=True,
                    )
        ...
        turn_rows.append(
            {
                ...
                "context_prime_seconds": (
                    prime_result["elapsed_seconds"]
                    if prime_result is not None
                    else None
                ),
                "context_prime_prompt_tokens": (
                    prime_result["prompt_tokens"] if prime_result is not None else None
                ),
